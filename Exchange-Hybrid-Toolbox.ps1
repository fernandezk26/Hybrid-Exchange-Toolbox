...
function Run-Toolbox {
	...
	function Create-User
	{
		...
		$OU = Get-ADOrganizationalUnit -Filter 'Name -like $branch' | Select-Object distinguishedName -ExpandProperty distinguishedname
		...
		$CN = Get-ADUser -Identity $samAccountName -properties DistinguishedName | Select-Object distinguishedname -ExpandProperty distinguishedName
		$TargetOU = Get-ADOrganizationalUnit -Filter 'Name -like $branch' | Select-Object distinguishedname -ExpandProperty distinguishedname
	...
	function ReEnable-UserAccess {
		...
		Set-MsolUserLicense -UserPrincipalName "$identity@yourdomain.com" -AddLicenses "yourdomain:ENTERPRISEPACK"

		#adding additional mobile license if title matches
		$title = Get-ADUser -Identity $username -properties title | Select-Object title -ExpandProperty title
		if ($title -match "Mobile" -or $title -match "Technician") {
			Set-MsolUserLicense -UserPrincipalName "$username@yourdomain.com" -AddLicenses "nsmseating:ENTERPRISE_MOBILE"
		}
	}
	...
	function Approve-MobileDevice {
		...
		Get-MobileDevice -Mailbox $identity | Format-List FriendlyName,Identity,DeviceAccessState,DeviceID
